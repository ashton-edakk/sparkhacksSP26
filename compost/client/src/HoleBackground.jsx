...
function HoleBackgroundImpl({
  ...
  // --- FIX START ---
  // We create a stable reference for the color.
  // Even if the parent passes a "new" [255,255,255] array every render,
  // this memo only updates if the actual numbers change.
  const stableRGB = useMemo(
    () => particleRGBColor,
    [particleRGBColor[0], particleRGBColor[1], particleRGBColor[2]]
  );
  // --- FIX END ---
  ...
  const initParticle = useCallback(
    (start = false) => {
      ...
      return {
        ...
        p: 0,
        r,
        // Use stableRGB here instead of particleRGBColor
        c: `rgba(${stableRGB[0]}, ${stableRGB[1]}, ${stableRGB[2]}, ${Math.random()})`,
      };
    },
    [stableRGB], // Dependency is now our memoized variable
  );
  ...
  return (
    <div
      ...
      className={cn(
        ...
        'before:content-[""] before:absolute before:top-1/2 before:left-1/2 before:block before:size-[140%] dark:before:[background:radial-gradient(ellipse_at_50%_55%,transparent_10%,black_50%)] before:[background:radial-gradient(ellipse_at_50%_55%,#064e3b_10%,#022c22_50%)] before:[transform:translate3d(-50%,-50%,0)]',
        'after:content-[""] after:absolute after:z-[5] after:top-1/2 after:left-1/2 after:block after:size-full after:[background:radial-gradient(ellipse_at_50%_75%,#10b981_20%,transparent_75%)] after:[transform:translate3d(-50%,-50%,0)] after:mix-blend-overlay',
        className,
      )}
      {...props}
    >
      ...
      <motion.div
        className={cn(
          ...
          'dark:[background:linear-gradient(20deg,#10b981,#059669_16.5%,#34d399_33%,#059669_49.5%,#10b981_66%,#34d399_85.5%,#047857_100%)_0_100%_/_100%_200%] [background:linear-gradient(20deg,#10b981,#059669_16.5%,#34d399_33%,#059669_49.5%,#10b981_66%,#34d399_85.5%,#047857_100%)_0_100%_/_100%_200%]',
        )}
...
const HoleBackground = React.memo(HoleBackgroundImpl);
